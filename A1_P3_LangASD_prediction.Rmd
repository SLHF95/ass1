...
```{r Load Libraries}
library(pacman)
p_load(lmerTest, modelr, stringr, tidyverse, caret, cvTools, Metrics)
```
...
save the data as a csv-file (probably not necessary):
```{r}
write.csv(df6, file = "ass1_LangASDtest.csv")
```
...
The model is a lot better at explaining the data than predicting the new data.
