...
model = lmer(CHI_MLU ~ 1 + VISIT + Diagnosis + (1+VISIT|SUBJ1), data = data)
summary(model)

model2 = lmer(CHI_MLU ~ 1 + VISIT * Diagnosis + (1+VISIT|SUBJ1), data = data)
summary(model2)

```

How would you evaluate whether the model is a good model?
There are many ways to asses how good a model is. 
Comparing model with null model - significant improvement?
How much variance is explained by the models? Take the one that explains the most
Looking at assumptions - explain both central and extreme data?
