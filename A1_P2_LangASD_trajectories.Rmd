...
[HERE GOES YOUR ANSWER]
The main effect Visit had a significant effect on the mean length of utterance produced by a child: (b = 0.10, SE = 0.03, t-value = 3.69, p < 0.001). Whether the child was diagnosed with ASD or not did not have a significant effect. 

There was a significant interaction effect of the visit and the diagnosis of the child, meaning that there is a significant difference in MLU for each visit depending on the diagnosis of the child. Not being diagnosed with ASD had a significant effect on MLU pr. visit: (b = 0.25, SE = 0.04, t-value = 6.72, p < 0.001).

In other words, typically developing children seem to produce longer utterances during visits than children with ASD.

*Is this a correct interpretation? Especially with the interaction?*
...
[HERE GOES YOUR ANSWER]
The main effect Visit had a significant effect on the mean length of utterance produced by the parent: (b = 0.10, SE = 0.03, t-value = 3.74, p < 0.001). The main effect diagnosis also had a significant effect: (b = 0.36, SE = 0.18, t-value = 2,02, p < 0.05). In other words, the main effects suggest that mothers speak more to their children over time, and that mothers of TD children speak more to their children than mothers of children with ASD.

There was no significant interaction effect of the visit and the diagnosis of the child, meaning that there is a no significant difference in MLU for each visit depending on the diagnosis of the child. The child not being diagnosed with ASD had no significant effect on parent MLU pr. visit: (b = 0.04, SE = 0.04, t-value = 1.10, p = 0.278)
...
[HERE GOES YOUR ANSWER]
My strategy was basically trial and error - trying different variables to see what would give me significant p-values, a higher R squared, but without too many variables (the simpler the better, right?).

My best model is model 4 with visit, diagnosis, types_CHI, and tokens_CHI as predictors. It has an R squared of 0.74.
...
```{r}
nullCHI_MLU = lmer(CHI_MLU ~ 1 + (1+VISIT|SUBJ1), data = data)

anova(nullCHI_MLU, mCHI_MLU)
anova(nullCHI_MLU, mCHI_MLUin)

```

In the interaction model, there is a less than 0.1% chance of finding a disrepancy between the observed and expected distributions that is at least 57.85.??

*How do I interpret the Chi squared? I found the above online but it still doesn't make sense to me.*
