...
```{r}
setwd("C:/Users/Bruger/Dropbox/0Aarhus Universitet/Experimental Methods III/Assignment 1/ass1")
```
...
```{r}
dfAll$SUBJ1 = as.numeric(as.factor(dfAll$SUBJ1))
```
...
dfAll$Gender = revalue(dfAll$Gender, c("1" = "F", "2" = "M"))

dfAll$Diagnosis = revalue(dfAll$Diagnosis, c("A" = "ASD", "B" = "TD"))
...
# mutate(mean_words = dfAll %>% 
#          group_by(VISIT) %>% 
#          summarise(mlu = mean(tokens_CHI, na.rm = TRUE)))
# 
# mutate(dfAll, mean_words_per_visit)
# 
# mean_words_per_visit = summarise(group_by(dfAll, VISIT), mean(tokens_CHI, na.rm = TRUE))
# 
# dfAll %>% group_by(VISIT) %>%
#   summarise(mlu = mean(tokens_CHI, na.rm = TRUE))
# 
# mutate(dfAll, mean_words_per_visit = summarise(mean(tokens_CHI), group_by(VISIT)))
